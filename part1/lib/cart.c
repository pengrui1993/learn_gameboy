...
static bool cart_mbc1(){ return BETWEEN(ctx.header->type,1,3);}
bool cart_battery();
...
void cart_setup_banking(){
    for(int i=0;i<16;i++){
        ctx.ram_banks[i] = 0;
        if((2==ctx.header->ram_size && 0 ==i)
            || (3==ctx.header->ram_size && i<4)
            || (4==ctx.header->ram_size && i<16)
            || (5==ctx.header->ram_size && i<8)
        ){
...
bool cart_load(char *cart)
{
    ...
    if(ctx.battery)cart_battery_load();
    return true;
}

// u8 cart_read(u16 address)
// { // for now,just rom read only type supported
//     return ctx.rom_data[address];
// }
// void cart_write(u16 address, u8 value)
// { // for now ,rom read only
//     printf("no impl cart_write(%04X)\n",address);
//     // NO_IMPL
// }
u8 cart_read(u16 address){ 
    if(!cart_mbc1()||address<0x4000)return ctx.rom_data[address];
    if(0xA000==(address&0xE000)){
...
void cart_write(u16 address, u8 value){  
    if(!cart_mbc1())return;
    if(address<0x2000)
        ctx.ram_enabled = (0xA==(value&0xF));
    if(0x2000 == (0xE000&address)){//rom bank number
        if(0==value)value = 1;
        value&=0b11111;
    ...
    if(0x4000 == (address&0xE000)){//ram bank number
        ctx.ram_bank_value = value&0b11;
        if(ctx.ram_banking){
            if(cart_need_save())
                cart_battery_save();
            ctx.ram_bank 
                = ctx.ram_banks[ctx.ram_bank_value];
        }
    }
    if(0x6000==(address&0xE000)){//banking mode select
...
void cart_battery_load(){
    char fn[2048];//filename
    sprintf(fn,"%s.battery",ctx.filename);
    FILE*fp = fopen(fn,"rb");
    if(!fp){
        fprintf(stderr,"FAILED TO OPEN WHEN LOAD: %s\n",fn);
        return;
    }
...
void cart_battery_save(){
    ...
    FILE*fp = fopen(fn,"wb");
    if(!fp){
        fprintf(stderr,"FAILED TO OPEN WHEN SAVE: %s\n",fn);
        return;
    }
    fwrite(ctx.ram_bank,0x2000,1,fp);
    fclose(fp);
}
bool cart_battery(){ //mbc1 only for now...
    return 3==ctx.header->type;
}
