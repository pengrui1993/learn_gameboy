...
static instruction instructions[0x100] ={
     [0x00] = {IN_NOP,AM_IMP}
    ,[0x01] = {IN_LD,AM_R_D16,RT_BC}//cmd:0x01
    ,[0x02] = {IN_LD,AM_MR_R,RT_BC,RT_A}//cmd:0x02
    ,[0x03] = {IN_INC,AM_R,RT_BC}
    ,[0x04] = {IN_INC,AM_R,RT_B}
    ,[0x05] = {IN_DEC,AM_R,RT_B}
    ,[0x06] = {IN_LD,AM_R_D8,RT_B}//cmd:0x06
    ,[0x07] = {IN_RLCA,AM_IMP}//TODO
...
//    printf("opcode:%02X start\n",opcode);
#if defined INS_DEBUG
    fprintf(stderr," opcode:%02X ",opcode);
#endif
    //if(IN_NONE==instructions[opcode].type)return NULL;
    return &instructions[opcode];
}

//see enum in_type,the sequence must be matched
